...
function sendMessage() {
    ...
    if (message !== '') {
        ...
        var loggedUserId = activeUserOrGroupElement.getAttribute("data-logged-user-id")
...
$(document).ready(function() {
    ...
    $.ajax({                       
        ...
        success: function (data) { 
            ...
            var loggedUserId = data.logged_user_id

            data.users_data.forEach(ele => {
                if (ele.room_user_full_name != " ") {
                    userDetailsTags += `<li><div data-room-id=${ele.chat_room_id} data-user-id=${ele.room_user_id} data-logged-user-id=${loggedUserId} onclick="selectItem(this)"><span>${ele.room_username}(${ele.room_user_full_name})</span></div></li>`
                }
                else{
                    userDetailsTags += `<li><div data-room-id=${ele.chat_room_id} data-user-id=${ele.room_user_id} data-logged-user-id=${loggedUserId} onclick="selectItem(this)"><span>${ele.room_username}</span></div></li>`
                }
            });

            data.groups_data.forEach(ele => {
                groupDetailsTags += `<li><div data-room-id=${ele.chat_room_id} data-group-id=${ele.group_id} data-logged-user-id=${loggedUserId} onclick="selectItem(this)"><span>${ele.group_name}</span></div></li>`
            });
